    ...
    <!-- 文章链接 -->
		...
		<div class="middle1_con">
            <div class="row-news clearfix">
              ...
              <c:forEach items="${fnc:getTopMenu()}" var="category" end="0" varStatus="status">
                      ...
                      <div class="move" id="move2">
                         ...
                         <c:forEach items="${fnc:getArticleList('1', category.id,7, '')}" var="article">
                          ...
                          <li><a href='${ctx}/view-${article.id}${fns:getUrlSuffix()}' target="_blank"><img class="srcoll"  src="${article.image}"></a></li>
                          </c:if>
                          <c:if test="${article.image==null||article.image=='' }">
        ...
        <c:forEach items="${fnc:getTopMenu()}" var="category" varStatus="status">
              <div class="middle1_1">
              	 <div class="news_div_top">
                 	 		 <c:if test="${status.index eq 0}">
                 	<img  src="${ctxStatic}/client/img/sqnews.png" class="img_center">
                 	 ${category.name}
                    <div class="more"><a target="_blank" style="text-decoration:none;color:black" href="${ctx}/alist-${category.id}-1-8${fns:getUrlSuffix()}">更多> >&emsp;</a></div>
                 	<c:forEach items="${fnc:getArticleList('1', category.id,7, '')}" var="article">
                       <div class="sqxnews">
                        <a href='${ctx}/view-${article.id}${fns:getUrlSuffix()}' target="_blank">
                 	...
                 	<img src="${ctxStatic}/client/img/sqdongtai.png" class="img_center">
                 	 ...
                 	 <c:forEach items="${fnc:getCommunityList(7)}" var="article">
                       <div class="sqxnews">
                        <a href="${ctx}/view-${article.id}.html" target="_blank">
                          <div style="float:left">●&nbsp;</div>
               ...
               <!--中间2轮播图-->
               <div class="carousel">
               	 <div class="slideGroup" style="z-index:0;">
			       <div class="parBd">
					<div class="slideBox">
						<a class="sPrev" href="javascript:void(0)"></a>
						<ul>
							 
                       <c:forEach items="${fnc:getMainFaceList('1', '社区风貌')}" var="article">
							<li>
								<div class="pic"><a target="_blank" href="javascript:void(0)"><img src="${article.image}" /></a></div>
							</li>
					   </c:forEach>	
               ...
               <div class="mar">
                <div class="middle_fuwu_tab">
                  	<div class="fuwu" onClick="fuwu(1,'${ctx}/business-list1-6')">
                     ...
                     <div class="content_li">
               	    <table class="table_fuwu">
                            ...
                            <c:forEach items="${bTypeList}" var="cv" end="2" varStatus="status">
                            	<td><a  target="_blank" href="${ctx}/business-list1-6.html?typeId=${cv.id}"><div class="groundImg" >${cv.name}</div></a></td>
                            </c:forEach>
                            </tr>
                            <tr>
