...
<body>
	<ul class="nav nav-tabs">
		...
		<li><a href="${ctx}/business/goods/blist?business.id=${goods.business.id}">商品管理</a></li>
		<li><a href="${ctx}/business/promotion/blist?business.id=${goods.business.id}">促销管理</a></li>
		<li class="active"><a href="${ctx}/business/goods/bform?id=${goods.id}&business.id=${goods.business.id}">商品<shiro:hasPermission name="business:goods:edit">${not empty goods.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="business:goods:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="goods" action="${ctx}/business/goods/bsave" method="post" class="form-horizontal">
		<form:hidden path="id"/>
