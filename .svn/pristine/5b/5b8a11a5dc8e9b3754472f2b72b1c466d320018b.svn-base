...
<head>
	...
	<script type="text/javascript">
		...
		var setDayInfo=	function (){
				...
				var date_info="今天是:"+date.getFullYear()+"年"+(date.getMonth()+1)+"月"+date.getDate()+" "+str;
			    var weather_info=" 天气:暂无数据";
			    $("#day_info").empty().html(date_info+weather_info);
			    //使用本地缓存
			    ...
			    if(storage.getItem('weather')){
			    	...
			    	if(hours<=1){
			    	    weather_info=" 天气:"+weather.weather+" "+weather.temperature+" "+weather.wind;
				        $("#day_info").empty().html(date_info+weather_info);
				     }else{
	...
	<div id="main">
		<div id="header" class="navbar navbar-fixed-top">
			<div class="navbar-inner"    ><!-- style="height: 70px;" -->
				<div class="brand"><%-- <img style="width: 16px;" src="${ctxStatic}/images/logo_90.png"/> --%><span id="productName">${fns:getConfig('productName')}</span></div>
				...
				<div style="float: right;font-size: 15px;font-family: 'Telex',sans-serif;color: #e8f0f7;       font-size: 13px; margin: 8px 20px;padding-top: 4px"><p id="day_info">今天是：2017年11月15日  星期五   天气：晴  21度  风力偏北2级</p></div>
	    ...
	    <div class="container-fluid">
		    ...
		    <div id="footer" class="row-fluid">
	           	Copyright &copy; ${fns:getConfig('copyrightYear')} <a href="${pageContext.request.contextPath}${fns:getFrontPath()}">${fns:getConfig('productName')}</a> - Powered By <a href="${fns:getConfig('bottom.link')}" target="_blank">${fns:getConfig('bottom.link.title')}</a> ${fns:getConfig('version')}
			</div>
		</div>
	</div>
	<script type="text/javascript"> 
